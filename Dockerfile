...
RUN sudo apt-get update && sudo apt-get upgrade && sudo apt-get dist-upgrade && sudo apt-get -y install gettext-base make gcc build-essential python python-pip && sudo pip install aws-shell
RUN cd ~ && git clone https://github.com/sillelien/dollar-parent.git __dp && cd __dp && mvn install && cd -
